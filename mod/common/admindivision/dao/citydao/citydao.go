...
func (dao *Dao) FindCodeByName(name, pcode string) string {
	sql, args := dao.Builder().Select("code").From(sqlkit.GetSchemaTable(dao.Schema, "city")).Where("name=?", name).Where("province=?", pcode).MustSql()
...
func (dao *Dao) ListByProvince(id class.String) []*model.City {
	sql, args := dao.Builder().Select(meta.GetColumns()).From(meta.GetTableName(dao.Schema)).Where("province=?", id).OrderBy("code").MustSql()
